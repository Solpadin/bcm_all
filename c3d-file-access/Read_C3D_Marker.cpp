...
// Position cursor to first data point
	res = fseek(infile, start_byte+offset, SEEK_SET);

	for (frame_num = 0; frame_num <= last_field-first_field; frame_num++) {
	// Read XYZ data for this frame
		for (i = 0; i < 3; i++) // for x, y, z
			res = fread(&val[frame_num][i], sizeof(short), 1, infile);
	// Read resiidual
		res = fread(&resi[frame_num], sizeof(char), 1, infile);
	// Read # cameras
		res = fread(&cam, sizeof(char), 1, infile);
	// Skip to the next frame
		res = fseek(infile, frame_length, SEEK_CUR);
	}
}
