...
//////////////////////////////////////
//...формирование геометрии включений;
void CHeat2D::testing1(CGrid * nd, int i, double * par, int id_variant)
{
   int N_elem = UnPackInts(get_param(NUM_QUAD)), N_max = UnPackInts(get_param(NUM_QUAD), 1), l, k;
	double R1 = get_param(NUM_GEOMT), R2 = get_param(NUM_GEOMT+1), pp[1], P[6], theta = M_PI*.25;
	if (par && par[0] < R2) theta -= acos(par[0]/R2); //...коррекция дуги окружности для взаимопроникающих включений;

//////////////////////////////////////
//...формирование геометрии включения;
